...
# MESA per-user setup: iRODS env, Data Store dotfiles (.gitconfig/.aws/.ssh),
# .env files, OSN mounts. Sourced (not set -e) so exported vars reach the
# desktop session and a failed mount never aborts startup.
set +e
